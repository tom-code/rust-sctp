...
impl SctpSocket {
    ...
    fn setsockopt2(&self, level: libc::c_int, option: libc::c_int, value: &[u8]) -> Result<()> {
        handle_libc_error(
        unsafe {
            libc::setsockopt(
                self.fd,
    ...
    pub fn bindx(&self, addresses: &[std::net::SocketAddr]) -> Result<()> {
        ...
        self.setsockopt2(SOL_SCTP, SCTP_SOCKOPT_BINDX_ADD, addrs.as_slice())
    }

    pub fn connectx(&self, addresses: &[std::net::SocketAddr]) -> Result<()> {
        let addrs = Self::socketaddrlist_to_os(addresses);
        self.setsockopt2(SOL_SCTP, SCTP_SOCKOPT_CONNECTX, addrs.as_slice())
    }
            ...
            unsafe {
                ...
                while !cmsghdr.is_null() {
                    if (*cmsghdr).cmsg_level == libc::IPPROTO_SCTP {
                        if (*cmsghdr).cmsg_type == SCTP_SNDRCV as i32 {
                            //sndrcv info
                            let data = libc::CMSG_DATA(cmsghdr);
                            let _info = data as *mut SndRcvInfo;
                            //println!("rcv info! {} {}", u32::from_be((*info).sinfo_ppid), (*info).sinfo_stream);
                        }
                    }
    ...
    /*fn write_fd(&self, buf: &[u8]) -> Result<usize> {
        let rv = unsafe {
